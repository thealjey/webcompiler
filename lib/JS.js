...
var JS = exports.JS = (function () {
  ...
  function JS() {
    ...
    var lintRules = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
    (0, _classCallCheck3.default)(this, JS);

    this.compiler = new _JSCompiler.JSCompiler(babelOptions);
    ...
    value: function typecheck(callback) {
      ...
      this.flow.run(function (flowErr, stdout) {
        ...
        if (!JSON.parse(stdout).passed) {
          return _this.flow.run(emptyFn, [], { stdio: 'inherit' });
        }
        callback();
      }, ['--json']);
    }
